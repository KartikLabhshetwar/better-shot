...
private extension DynamicNotch {
    ...
    func initializeWindow(screen: NSScreen, orderFront: Bool = true) {
        ...
        let view = NotchHostingView(rootView: NotchContentView(dynamicNotch: self, style: style))
        view.visibleContentFrame = { [weak self] in self?.contentFrame ?? .zero }
        view.sizingOptions = []
...
private final class NotchHostingView<Content: View>: NSHostingView<Content> {
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        guard visibleContentFrame().contains(convert(point, from: superview)) else { return nil }
