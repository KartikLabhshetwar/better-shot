  ...
  return (
    <div className="relative w-full">
      <div
        ref={stageRef}
        className={cn(
          "relative overflow-hidden rounded-2xl bg-[#0b0b0d] shadow-2xl ring-1 ring-white/10",
      ...
      >
        {failed ? (
          <div className="absolute inset-0 grid place-content-center gap-2 p-6 text-center">
            <p className="text-sm font-medium text-white">This screenshot could not be loaded</p>
            <p className="text-xs text-white/60">The file may have been moved or deleted.</p>
          </div>
        ) : (
          <img
            src={src}
        ...
        {!failed && (
          <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-0.5 rounded-full border border-white/10 bg-black/60 p-1 text-white opacity-0 shadow-lg backdrop-blur-xl transition-opacity duration-200 focus-within:opacity-100 hover:opacity-100 motion-reduce:transition-none">
            ...
            <button
              ...
              className="min-w-14 rounded-full px-2 py-1 text-xs font-semibold tabular-nums text-white/85 transition-colors duration-100 hover:bg-white/15 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
            >
...
function ZoomButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="grid size-8 place-content-center rounded-full text-white/85 transition-colors duration-100 hover:bg-white/15 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 disabled:opacity-30"
    >
      {children}
    </button>
  )
