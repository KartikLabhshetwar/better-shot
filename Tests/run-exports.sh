...
if [[ "${BETTERSHOT_CHECK_EDITOR_SCENES:-0}" == "1" ]]; then
    swiftc -parse-as-library -module-cache-path .build/ExportCheckModules \
        ...
        "${objects[@]}" ".build/Build/Products/$configuration/DockProgress.o" ".build/Build/Products/$configuration/TourKit.o" \
...
swiftc -parse-as-library -module-cache-path .build/ExportCheckModules \
    -I ".build/Build/Products/$configuration" Tests/ExportIntegration.swift Tests/EditorUIIntegration.swift Tests/ImageTransformIntegration.swift Tests/Recording3DIntegration.swift \
    "${objects[@]}" ".build/Build/Products/$configuration/DockProgress.o" ".build/Build/Products/$configuration/TourKit.o" \
