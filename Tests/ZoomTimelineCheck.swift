...
nonisolated struct ZoomCue: Identifiable, Codable, Equatable, Sendable {
    ...
    init(
        ...
        self.pinnedPoint = pinnedPoint
        self.boundsBias = boundsBias
...
nonisolated struct PointerPressEvent: Codable, Sendable {
    enum Phase: String, Codable, Sendable { case down }
    var time: TimeInterval
    var x: Double
    var y: Double
    var phase: Phase
...
nonisolated struct ViewportTimeline: Sendable {
    ...
    private struct PointerSample: Sendable {
        enum Kind: Sendable { case travel, press }
        var time: TimeInterval
        var point: CGPoint
        var kind: Kind
    }
...
func assertClamped(_ frame: ViewportFrame, label: String) {
    let halfExtent = 1 / (2 * max(frame.magnification, 1))
    assert(frame.anchor.x - halfExtent >= -1e-6, "\(label): left edge escapes frame (\(frame))")
    assert(frame.anchor.x + halfExtent <= 1 + 1e-6, "\(label): right edge escapes frame (\(frame))")
    assert(frame.anchor.y - halfExtent >= -1e-6, "\(label): top edge escapes frame (\(frame))")
    assert(frame.anchor.y + halfExtent <= 1 + 1e-6, "\(label): bottom edge escapes frame (\(frame))")
}

do {
    let timeline = ViewportTimeline.build(cues: [], capture: PointerCaptureFile(), duration: 10)
    for t in stride(from: 0.0, through: 10.0, by: 0.7) {
        let frame = timeline.frame(at: t)
        assert(frame == .identity, "expected identity with no cues at t=\(t), got \(frame)")
    }
}

do {
    assert(ViewportTimeline.identity.frame(at: 0) == .identity, "identity timeline (models zoomEnabled == false) must stay identity")
    assert(ViewportTimeline.identity.frame(at: 5) == .identity, "identity timeline (models zoomEnabled == false) must stay identity")
}

do {
    let cue = ZoomCue(start: 1, end: 4, zoom: 2.0, anchorMode: .pinnedAnchor, pinnedPoint: CGPoint(x: 0.9, y: 0.1))
    let timeline = ViewportTimeline.build(cues: [cue], capture: PointerCaptureFile(), duration: 6)

    let before = timeline.frame(at: 0)
    assert(abs(before.magnification - 1) < 0.05, "expected near-identity before cue starts, got \(before.magnification)")

    let settled = timeline.frame(at: 3.5)
    assert(abs(settled.magnification - 2.0) < 0.05, "expected settled magnification near 2.0, got \(settled.magnification)")

    for t in stride(from: 0.0, through: 6.0, by: 0.05) {
        assertClamped(timeline.frame(at: t), label: "pinned cue at t=\(t)")
    }
}

do {
    let cue = ZoomCue(start: 0, end: 3, zoom: 3.0, anchorMode: .pinnedAnchor, pinnedPoint: CGPoint(x: 0, y: 0))
    let timeline = ViewportTimeline.build(cues: [cue], capture: PointerCaptureFile(), duration: 3)
    for t in stride(from: 0.0, through: 3.0, by: 0.02) {
        assertClamped(timeline.frame(at: t), label: "corner cue at t=\(t)")
    }
    let late = timeline.frame(at: 2.5)
    assert(late.anchor.x >= 1 / (2 * late.magnification) - 1e-6, "corner anchor did not clamp on x")
    assert(late.anchor.y >= 1 / (2 * late.magnification) - 1e-6, "corner anchor did not clamp on y")
}

do {
    let cue = ZoomCue(start: 0, end: 5, zoom: 2.5, anchorMode: .pinnedAnchor, pinnedPoint: CGPoint(x: 0.5, y: 0.5))
    let timeline = ViewportTimeline.build(cues: [cue], capture: PointerCaptureFile(), duration: 5)

    let stepRate = ViewportTimeline.stepRate
    let sampleTime: TimeInterval = 2.0031
    let index = Int(sampleTime * stepRate)
    let baseTime = Double(index) / stepRate
    let nextTime = Double(index + 1) / stepRate

    let a = timeline.frame(at: baseTime)
    let b = timeline.frame(at: nextTime)
    let mid = timeline.frame(at: sampleTime)

    let fraction = (sampleTime - baseTime) / (nextTime - baseTime)
    let expectedMagnification = a.magnification + (b.magnification - a.magnification) * fraction
    assert(abs(mid.magnification - expectedMagnification) < 1e-6, "interpolation mismatch: \(mid.magnification) vs \(expectedMagnification)")
}

do {
    let cue = ZoomCue(start: 2, end: 4, zoom: 2.0, anchorMode: .pinnedAnchor, pinnedPoint: CGPoint(x: 0.5, y: 0.5))
    let timeline = ViewportTimeline.build(cues: [cue], capture: PointerCaptureFile(), duration: 4)

    let atStart = timeline.frame(at: 2.0)
    let justAfterStart = timeline.frame(at: 2.2)
    assert(justAfterStart.magnification >= atStart.magnification, "magnification should not decrease right after cue starts")

    let atEnd = timeline.frame(at: 4.0)
    let beyondEnd = timeline.frame(at: 999)
    assert(atEnd == beyondEnd, "frame(at:) should clamp to the timeline duration")
}

do {
    let mergedCue = ZoomCue(start: 0.7, end: 5.0, zoom: 1.6, anchorMode: .pointerAnchor, pinnedPoint: CGPoint(x: 0.3, y: 0.3))
    let timeline = ViewportTimeline.build(cues: [mergedCue], capture: PointerCaptureFile(), duration: 6)
    for t in stride(from: 0.0, through: 6.0, by: 0.1) {
        assertClamped(timeline.frame(at: t), label: "merged cue at t=\(t)")
    }
}

print("ZoomTimelineCheck: all assertions passed")
