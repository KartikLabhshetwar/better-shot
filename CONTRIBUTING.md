...
Close up intentionally uses a gentler, centered camera and broader focus region;
its framing has a separate check across landscape, square, and portrait canvases.
Scene timing and the other 12 looks still match upstream. Stored project poses
are not rewritten; the refinement applies when generating or selecting new shots.
