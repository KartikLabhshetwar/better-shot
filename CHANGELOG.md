...
This upcoming release consolidates the unpublished 0.4.3 and 0.4.4 work.
...
- **Keep screenshots in the deck until saved**: A new Capture setting stops captures from landing in the save folder and Recent Captures right away. They wait in the deck instead, where cards never time out, and are saved the moment you press Save, Copy, drag one out, pin it, or open it in the editor; Save All saves every card at once, while closing a card, Trash, or Clear All deletes what was never saved. Whatever is left over is cleaned up on the next launch ([#76](https://github.com/KartikLabhshetwar/better-shot/issues/76))
- **All-in-one bar**: The floating bar appears at launch and `⌘⇧5` reopens it. It carries the screenshot actions too: region, window, screen, OCR and color picker sit beside the recording sources, so one shortcut reaches everything (#122)
...
- **Preview card edge margin and controls that scale**: A "Keep it off the edge by" slider (0 to 48pt, default 20) sets how far the after-capture deck sits from the screen corner, and the hover buttons, Copy/Save pills and video play badge now grow with the Medium and Large card sizes instead of staying tiny ([#124](https://github.com/KartikLabhshetwar/better-shot/pull/124), thanks [@BradleyAllanDavis](https://github.com/BradleyAllanDavis))
...
- **Full-screen preference**: Image and video editors can open in full screen, independently of the capture workflow.
- **Export defaults**: Recording settings offer render speed, resolution, and codec defaults.

### Changed

- **Direct screenshot shortcut**: `⌘⇧2` starts region capture; `⌘⇧5` opens the single shared capture/recording bar, which appears automatically at launch. Saved default shortcuts migrate while custom bindings and disabled states are preserved.
- **Uncluttered image sidebar**: Canvas settings live only in Background, aspect ratio stays in the toolbar, and manual/automatic redaction share the toolbar's Redact control. Cloud sharing uses an upload-to-cloud icon.

- **Simpler editors**: Image and video inspectors sit on the left with flat grey sections, standard sliders, and exact value entry. Detailed zoom framing and image styling stay under Advanced. Nested effect cards and redundant amount presets are removed.
- **Classic glass controls**: The shared capture bar uses labeled modes. During recording, Stop and the red timer sit together, followed by Pause, Restart, and Discard, separated by dividers on a frosted surface. Restart and Discard ask before deleting footage.
- **Focused image toolbar**: Annotation tools, color, and style controls share one row; zoom and sharing sit in the footer.
- **Simpler timeline**: Scissors sits at the far left, playback stays in the center, and a slim zoom slider sits on the right. Blue zoom segments sit above video thumbnails with a red playhead. Secondary editing actions remain in the timeline menu and existing keyboard shortcuts are retained.
- **Four video sections**: Background, Camera, Effects, and Zoom & Clips keep recording tools together. Crop and Censor live in Effects, without a duplicate floating toolbar group.
- **Recording options**: Camera, microphone, system audio, and teleprompter remain available from the common capture bar. Screenshot and recording countdowns are configured from Timer.
- **Recent Captures**: Screenshots and recordings share the existing recent-capture navigation. The interim Media Gallery and Recording Projects browser are removed; captures, share links, and editable projects are preserved.
- **Rendering performance**: Existing GPU rendering, shared Core Image contexts, hardware encoding, cached backgrounds, and reusable export deliverables are preserved. Timeline drawing stays limited to the visible range and thumbnail sampling remains deferred during navigation.
- **Settings**: Capture and recording settings explain their options inline, sharing setup keeps its fields editable, and System appearance follows macOS.

### Fixed

- Zoom segments resize independently of timeline magnification, and interrupted edits close their undo group. Added zooms on imported videos enable zoom playback immediately.
- Timeline navigation retains pointer/playhead anchoring, pinch and modifier-scroll, short-recording zoom limits, Fit, and the native lane-width cap.
- Image/video crop, masking, zoom edits, and undo/redo retain their existing model and export paths. Failed image exports preserve the destination, and video export handles writer errors and cancellation.
- Recent captures reopen after menu tracking ends, recordings use cached posters, and unreadable thumbnails remain actionable.
- Capture filenames use full UUIDs, failed writes clean up staging files, and failed imports never reuse an earlier screenshot. Deleted captures cannot return after delayed rendering.
- History recognizes equivalent file paths while preserving similarly named sibling projects.
- Screenshot and recording editor preferences remain independent. Shutter sound uses the same setting throughout the app, and disconnected devices remain visible in settings.
- Build pipelines stop on compiler errors. Automated checks avoid the user's R2 Keychain and verify editor, timeline, crop, mask, and export behavior.
...
- **The preview deck respects Bottom Left**: The card was always aligned to the trailing edge, so with the overlay set to Bottom Left it floated a panel's width in from the screen instead of one margin. Alignment now follows the chosen corner ([#124](https://github.com/KartikLabhshetwar/better-shot/pull/124), thanks [@BradleyAllanDavis](https://github.com/BradleyAllanDavis))
