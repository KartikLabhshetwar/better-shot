...
- **Cursor smoothing**: Leave “The mouse cursor” out in Settings, Recording and the editor draws its own pointer from the path you actually moved. A spring chases that path, so a shaky hand glides, the pointer never trails behind the motion, and a click lands exactly on the spot instead of arriving late. Pick Loose, Natural or Snappy in the Motion tab and set the size; the preview and the exported file draw the same cursor, scaled by the zoom like the rest of the picture
