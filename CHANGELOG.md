...
- **3D timeline workflow:** a visible **+ Add** menu offers Zoom and 3D Shot; empty cut-marker rows collapse so 3D segments sit directly below the filmstrip. All eight moves and five angles now follow the reference camera framing and drift, with independent screen fold, distance, and lens controls, linear preset motion, and weighted scenes. Previously saved poses retain their appearance.
- **3D depth and animation:** Camera, Blur, and Keyframes tabs add radial, directional, and tilt-shift focus, bokeh highlights, and per-property keyframes with draggable Bézier handles and numeric editing. Curves resize with their shot and survive undo, saving, export, and sharing. Preview reuses the export GPU compositor with bounded frames in flight; older saved shots keep their look.
